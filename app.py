...
def app_today():
    """Return today using the application/user timezone (India), not UTC/server date."""
...
def index():
    ...
    if requested_week:
        ...
        try:
            selected_year = int(request.args.get("year", app_today().year))
            selected_month = min(12, max(1, int(request.args.get("month", app_today().month))))
            ...
            week_start = monday_of(app_today())
